#include "nut_bolt_detection.h"

namespace NutBoltDetection
{
    NutBoltDetection::Detector::fps_calculator NutBoltDetection::Detector::fps[16];
    char *NutBoltDetection::Detector::TRACKER_CONFIG_FILE = nullptr;
    char *NutBoltDetection::Detector::PGIE_NUT_BOLT_DETECTOR_CONFIG_FILE_PATH = nullptr;

    void
    Detector::update_fps(gint id)
    {
    ...
    int
    Detector::create_input_sources(gpointer pipe, gpointer mux, guint num_sources)
    {
    ...
    void
    Detector::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
    ...
    GstPadProbeReturn
    Detector::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                                         gpointer u_data)
    {
    ...
    gboolean
    Detector::bus_call(GstBus *bus, GstMessage *msg, gpointer data)
    {
    ...
    void
    Detector::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data)
    {
    ...
    void
    Detector::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data)
    {
    ...
    GstElement *
    Detector::create_source_bin(guint index, gchar *uri)
    {
    ...
    gchar *
    Detector::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path)
    {
    ...
    gboolean
    Detector::set_tracker_properties(GstElement *nvtracker)
    {
        gboolean ret = FALSE;
        GError *error = NULL;
        gchar **keys = NULL;
    ...
    int
    Detector::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
                                           GstElement *nvtracker, GstElement *sink, GstElement *tiler)
    {
        ...
        g_object_set(G_OBJECT(pgie_yolo_detector),
                     "config-file-path", PGIE_NUT_BOLT_DETECTOR_CONFIG_FILE_PATH, NULL);

        // Override batch-size of pgie_yolo_detector
        g_object_set(G_OBJECT(pgie_yolo_detector), "batch-size", num_sources, NULL);

        // Check if Engine Exists
        if (filesystem::exists(filesystem::path(
                NutBoltDetection::Detector::PGIE_NUT_BOLT_ENGINE_PATH)))
        {

            g_object_set(G_OBJECT(pgie_yolo_detector),
                         "model-engine-file", NutBoltDetection::Detector::PGIE_NUT_BOLT_ENGINE_PATH.c_str(), NULL);
        }
        else
        {
            // Using std::format (C++20 and later)
            cout << format("YOLO Engine for batch-size: {} and compute-mode: {} not found.", num_sources, COMPUTE_MODE) << endl;
            return EXIT_FAILURE;
        }

        // Set necessary properties of the tracker element
        if (!NutBoltDetection::Detector::set_tracker_properties(nvtracker))
        {
    ...
    void Detector::setPaths(guint num_sources)
    {
        ...
        PGIE_NUT_BOLT_ENGINE_PATH =
            format("model_b{}_gpu0_{}.engine", num_sources, COMPUTE_MODE);
    }

    NutBoltDetection::Detector::~Detector()
    {
...
int main(int argc, char *argv[])
{
    ...
    ctx = g_option_context_new("NutBoltRingDetection DeepStream App");
    group = g_option_group_new("NutBoltRingDetection", NULL, NULL, NULL, NULL);
    g_option_group_add_entries(group, detector.entries);
    ...
    gint sources = detector.create_input_sources(pipeline, streammux, num_sources);
    if (sources == -1)
    {
        return -1;
    }
...
/* Redner OSD Output */
    ...
    int fail_safe = detector.configure_element_properties(num_sources, streammux, pgie_yolo_detector,
    ...
    bus_watch_id = gst_bus_add_watch(bus, detector.bus_call, loop);
...
/* Set up the pipeline */
    ...
    else
    {
        gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                          detector.tiler_src_pad_buffer_probe, NULL, NULL);
    }
